...
func (lm loggingMiddleware) UpdateRead(ctx context.Context, id string, ts time.Time) (err error) {
	err = lm.stor.UpdateRead(ctx, id, ts)
	switch err {
	case nil:
		lm.log.Debug(fmt.Sprintf("UpdateRead(%s, %s)", id, ts))
	default:
		lm.log.Error(fmt.Sprintf("UpdateRead(%s, %s): err=%s", id, ts, err))
	}
	return
}
